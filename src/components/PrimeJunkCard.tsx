...
export default function PrimeJunkCard({ part, partData, onDecrement, onIncrement }: PrimeJunkCardProps) {
    ...
    return (
        <div className="prime-junk-card">
            <img className="prime-junk-image" src={`https://cdn.warframestat.us/img/${part.imageName}`}></img>
            <h3 className="prime-junk-title"><strong>{part.parentName}</strong> {part.componentName}</h3>
            <div className="prime-junk-counter">
                <button className="prime-junk-decrement" disabled={count <= 0} onClick={() => onDecrement(part)}>-</button>
                <h4 className="prime-junk-value">{count}</h4>
